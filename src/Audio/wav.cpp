...
// チャンネル数を返す
unsigned int Wav::channel() const { return info.ch; }

// データがステレオならtrueを返す
bool Wav::isStereo() const { return info.ch == 2; }

// サンプリングレートを返す
unsigned int Wav::sampleRate() const { return info.sample_rate; }

// データサイズ(バイト数)を返す
unsigned int Wav::size() const { return info.size; }
...
unsigned int Wav::getValue(const char* ptr, const unsigned int num) {
	unsigned int value = 0;

	for (unsigned int i = 0; i < num; ++i, ++ptr) {
    // TIPS:int型より小さい型はint型に拡張されてから計算されるので8bit以上シフトしても問題ない
		value = value + (static_cast<unsigned char>(*ptr) << (i * 8));
  }
...
bool Wav::searchChunk(std::ifstream& fstr, const char* chunk) {
  ...
  while (1) {
    ...
    char data[4];
    fstr.read(data, 4);
		unsigned int chunk_size = getValue(data, 4);
...
// チャンクのサイズを取得
unsigned int Wav::getChunkSize(std::ifstream& fstr) {
  char data[4];
  fstr.read(data, 4);
...
bool Wav::analyzeWavFile(Info& info, std::ifstream& fstr) {
  ...
  if (!searchChunk(fstr, "fmt ")) {
	...
	unsigned int chunk_size = getChunkSize(fstr);
